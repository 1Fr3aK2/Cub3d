...
/*   Updated: 2025/12/09 16:52:43 by raamorim         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
bool	check_load_textures(char *floor, char *ceiling, t_map *map)
{
	if (!map || !floor || !ceiling)
		return (false);
	if (!(&map->assets.textures[N]) || !(&map->assets.textures[S])|| !(&map->assets.textures[EA]) || !(&map->assets.textures[WE]))
		return (false);
	if (!ceiling || !floor)
		return (false);
	return (true);
...
bool	set_texture(char *line, char **floor, char **ceiling, t_map *map)
{
	...
	if (split[0])
	{
		...
		if (&map->assets.textures[N] == NULL && ft_strncmp(split[0], "NO", 3) == 0)
			map->assets.textures[N].path = ft_strdup(split[1]);
		else if (&map->assets.textures[S] == NULL && ft_strncmp(split[0], "SO", 3) == 0)
			map->assets.textures[S].path = ft_strdup(split[1]);
		else if (&map->assets.textures[WE] == NULL && ft_strncmp(split[0], "WE", 3) == 0)
			map->assets.textures[WE].path = ft_strdup(split[1]);
		else if (&map->assets.textures[EA] == NULL && ft_strncmp(split[0], "EA", 3) == 0)
