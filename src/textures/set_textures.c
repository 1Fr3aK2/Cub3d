...
bool	set_assets_texture(t_assets *assets, char **path, char **floor,
		char **ceiling)
{
	if (!assets || !path || !*path)
		return (false);
	...
	else if (!assets->textures[S].path && ft_strncmp(path[0], "SO", 3) == 0)
		return (load_texture(&assets->textures[S], path[1]));
	else if (!assets->textures[WE].path && ft_strncmp(path[0], "WE", 3) == 0)
		return (load_texture(&assets->textures[WE], path[1]));
	else if (!assets->textures[EA].path && ft_strncmp(path[0], "EA", 3) == 0)
